...
class NetworkRequests {
    ...
    func grabImageAt(url: URL, _ callback: @escaping ((UIImage?)->Void)) {
        let httpURL = URL(string: url.absoluteString.replacingOccurrences(of: "https://", with: "http://"))!
        let task = session.dataTask(with: httpURL) { (data, response, error) in
            self.inProgress -= 1
            guard let data = data else {
                callback(nil)
                return
